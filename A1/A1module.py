...
class classifier:
    ...
    def _optimalCNN(self):
        ...
        model.add(keras.layers.Flatten())
        model.add(keras.layers.Dense(192, activation='relu', kernel_regularizer='l2'))
        ...
        model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4),
                      ...
                      metrics=['accuracy'])
        return model
    def train_optimalCNN(self,X,Y,testX,testY,batch_size=64,epochs=300):
...
# # Classifier.loss_monitor((SVC(kernel='rbf', C=1000,gamma=0.0001)), trainX, trainY,
# #              valX, valY,
# #              label='SVM(rbf), C=1000, gamma=0.0001', color=color)
# # color = next(ax._get_lines.prop_cycler)['color']
# # Classifier.loss_monitor((KNeighborsClassifier(n_neighbors=21)), trainX, trainY,
# #              valX, valY,
# #              label='KNN, n_neighbors=21', color=color)
# # color = next(ax._get_lines.prop_cycler)['color']
# # Classifier.loss_monitor((RandomForestClassifier(n_estimators=71)), trainX, trainY,
# #              valX, valY,
# #              label='Random forest, n_estimators=71', color=color)
# #
# # plt.ylim([0.35,1.05])